...
namespace aspnetcore__odbc.Controllers
{
    ...
    public class ProductOdbcController : ControllerBase
    {
        ...
        public ProductOdbcController(IConfiguration configuration)
        {
            connectionString = configuration.GetConnectionString("ODBCConnection");
        }
        ...
        public IActionResult CreateProduct(ProductDto productDto)
        {
            try
            {
                using (var connection = new OdbcConnection(connectionString))
                {
                    ...
                    command.CommandText = "INSERT INTO products (Name, Brand, Category, Price, Description) VALUES (?, ?, ?, ?, ?)";
        ...
        public IActionResult GetProductById(int id)
        {
            ...
            try
            {
                using (var connection = new OdbcConnection(connectionString))
                {
                    connection.Open();
                    var command = connection.CreateCommand();
                    command.CommandText = "SELECT * FROM products WHERE id = ?";
        ...
        public IActionResult UpdateProduct(int id, ProductDto productDto)
        {
            try
            {
                using (var connection = new OdbcConnection(connectionString))
                {
                    connection.Open();
                    var command = connection.CreateCommand();
                    command.CommandText = "UPDATE products SET Name =?, Brand = ?, Category = ?, Price = ?, Description = ? WHERE id = ?";
                    command.Parameters.AddWithValue("@name", productDto.Name);
                    command.Parameters.AddWithValue("@brand", productDto.Brand);
                    command.Parameters.AddWithValue("@category", productDto.Category);
                    command.Parameters.AddWithValue("@price", productDto.Price);
